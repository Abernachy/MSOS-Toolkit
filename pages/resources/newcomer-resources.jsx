...
import { newcomerResources } from '../../src/utils/resources/newcomerResources'

const NewcomerResources = () => {
	const [userAssets, setUserAssets] = useState(newcomerResources)

	const stuff = useMemo(
		() => resourceBuilder(userAssets, 'type'),
		[userAssets]
	)
	const [newcomerResourcesList, setNewcomerResourcesList] = useState(stuff)

	return (
		<div>
			<section className={style.topHeader}>
				<h1>Newcomer Resources</h1>
			</section>
			<section className={style.mainBody}>
				<p>
					Below are resources for our new or future members of the
					squadron. If you believe a resource is missing that should be
					added, please do not hesitate to reach out to us.
				</p>
				<div className={style.lineDivider} />
			</section>
			<section className={style.resourcesSection}>
				{newcomerResourcesList.map((item, index) => (
